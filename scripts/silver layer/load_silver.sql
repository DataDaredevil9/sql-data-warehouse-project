--Inserting the cleaned & transformed data into silver layer
...
BEGIN
	BEGIN TRY
		...
		PRINT '===========================================================';
		PRINT 'INSERTING THE CLEANED & TRANSFORMED DATA INTO SILVER LAYER';
		PRINT '===========================================================';
		SET @start_time=GETDATE();
		PRINT '>>TRUNCATING THE TABLE : silver.crm_cst_info';
		TRUNCATE TABLE silver.crm_cst_info;
		PRINT '>> INSERTING INTO TABLE : silver.crm_cst_info';
		...
		FROM 
		bronze.crm_cst_info WHERE cst_id IS NOT NULL) t
		WHERE flag_test=1;
		...
		CASE UPPER(TRIM(prd_line))
			 ...
			 WHEN 'T' THEN 'Touring'
			 ELSE 'N/A'
		END prd_line,--Map product lines code to descriptive values
		...
		END AS sls_due_dt,
		-- 3 Rules in transformation
		--1. Is sales is NUll ,zero,negative or not equals to formula calculate it using quantity*price
		--2. If price is null or zero calculate it using sales/quantity
